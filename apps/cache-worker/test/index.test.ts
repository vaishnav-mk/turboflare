...
describe("cache worker", () => {
  ...
  it("reports and purges internal team artifacts", async ({ expect }) => {
    ...
    await admin.env.ARTIFACTS.put(
      `v1/team/${TEAM_ID}/branch/pr-1/artifact/branch`,
      new Uint8Array([5]),
    );
    await admin.env.ARTIFACTS.put(`v1/team/${OTHER_TEAM_ID}/artifact/three`, new Uint8Array([4]));

    const stats = await handleRequest(
      internalRequest(`/internal/teams/${TEAM_ID}/stats`, admin),
      admin.env,
      createExecutionContext(),
    );
    ...
    const purge = await handleRequest(
      internalRequest(`/internal/teams/${TEAM_ID}/purge-all`, admin, { method: "POST" }),
      admin.env,
      createExecutionContext(),
    );
    expect(purge.status).toBe(200);
    expect(await purge.json()).toEqual({ deleted: 3, team: TEAM_ID, truncated: false });

    expect(await admin.env.ARTIFACTS.head(`v1/team/${TEAM_ID}/artifact/one`)).toBeNull();
    expect(
      await admin.env.ARTIFACTS.head(`v1/team/${TEAM_ID}/branch/pr-1/artifact/branch`),
    ).toBeNull();
    expect(
      await admin.env.ARTIFACTS.head(`v1/team/${OTHER_TEAM_ID}/artifact/three`),
    ).not.toBeNull();
  });
  ...
  it("creates lists and revokes internal tokens", async ({ expect }) => {
    ...
    const admin = internalAdmin({
      ARTIFACTS: (workerEnv as unknown as Env).ARTIFACTS,
      TOKEN_DB: tokenDb as unknown as D1Database,
    } satisfies Env);
    ...
    expect(await list.json()).toEqual({
      tokens: [
        { expiresAt, id: "ci-token", revokedAt: null, scopes: ["read", "write"], teams: [TEAM_ID] },
      ],
    });

    const revoke = await handleRequest(
      internalRequest("/internal/tokens/ci-token/revoke", admin, { method: "POST" }),
      admin.env,
      createExecutionContext(),
    );
    expect(revoke.status).toBe(200);
    expect(await revoke.json()).toEqual({
      revoked: { id: "ci-token", revokedAt: expect.any(String) },
    });
  ...
  it("returns a conflict for duplicate internal token ids", async ({ expect }) => {
    const tokenDb = new TokenAdminDb();
    const admin = internalAdmin({
      ARTIFACTS: (workerEnv as unknown as Env).ARTIFACTS,
      TOKEN_DB: tokenDb as unknown as D1Database,
    ...
    expect(await second.json()).toEqual({
      error: { code: "already_exists", message: "Token id or token value already exists" },
    });
  });
  ...
  it("purges expired artifacts through the internal route", async ({ expect }) => {
    ...
    const response = await handleRequest(
      internalRequest("/internal/artifacts/purge-expired", admin, { method: "POST" }),
      admin.env,
      createExecutionContext(),
    );
  ...
  it("reports configured cache status variants", async ({ expect }) => {
    for (const status of Object.values(CacheStatus)) {
      const response = await handleRequest(
        new Request(`${BASE_URL}${ARTIFACT_STATUS_PATH}`, {
  ...
  it("rejects rate-limited artifact requests", async ({ expect }) => {
    ...
    const response = await handleRequest(
      ...
      {
        ARTIFACTS: (workerEnv as unknown as Env).ARTIFACTS,
        RATE_LIMITER: rateLimiter as unknown as RateLimit,
        TURBO_TOKEN: TOKEN,
      },
      createExecutionContext(),
    );
  ...
  it("fails explicitly when the rate limiter binding fails", async ({ expect }) => {
    const response = await handleRequest(
      new Request(`${BASE_URL}${ARTIFACTS_PATH}/${randomArtifactId()}?teamId=${TEAM_ID}`, {
        headers: { Authorization: `Bearer ${TOKEN}` },
      }),
    ...
    expect(response.status).toBe(503);
    expect(await response.json()).toEqual({
      error: { code: "unavailable", message: "Rate limiter is unavailable" },
    });
  });
  ...
  it("rejects artifacts above the configured upload quota", async ({ expect }) => {
    ...
    expect(await response.json()).toEqual({
      error: {
        ...
        message: "Artifact upload exceeds configured size limit",
      },
    });
  });

  it("rejects streamed artifacts above the upload quota without content length", async ({
    expect,
  }) => {
    const response = await handleRequest(
      new Request(`${BASE_URL}${ARTIFACTS_PATH}/${randomArtifactId()}?teamId=${TEAM_ID}`, {
  ...
  it("streams artifact uploads and downloads through R2", async ({ expect }) => {
    ...
    const stored = await (workerEnv as unknown as Env).ARTIFACTS.head(
      `v1/team/${TEAM_ID}/artifact/${artifactId}`,
    );
  ...
  it("requires signed artifact metadata when signature policy is require", async ({ expect }) => {
    ...
    expect(await unsigned.json()).toEqual({
      error: { code: "signature_required", message: "Signed artifact metadata is required" },
    });
  ...
  it("isolates branch artifacts when branch cache policy is isolated", async ({ expect }) => {
    ...
    expect(put.status).toBe(200);
    expect(
      await env.ARTIFACTS.head(`v1/team/${TEAM_ID}/branch/feature%2Fa/artifact/${artifactId}`),
    ).not.toBeNull();
  ...
  it("falls back to main artifacts for pull request branch reads", async ({ expect }) => {
    ...
    await env.ARTIFACTS.put(`v1/team/${TEAM_ID}/artifact/${artifactId}`, new Uint8Array([9]), {
      httpMetadata: { contentType: "application/octet-stream" },
    });

    const get = await handleRequest(
      new Request(`${BASE_URL}${ARTIFACTS_PATH}/${artifactId}?teamId=${TEAM_ID}&branch=pr-1`, {
        headers: { Authorization: `Bearer ${TOKEN}` },
      }),
      env,
      createExecutionContext(),
  ...
  it("rejects pull request writes in read-only branch policy", async ({ expect }) => {
    const response = await handleRequest(
      new Request(
        `${BASE_URL}${ARTIFACTS_PATH}/${randomArtifactId()}?teamId=${TEAM_ID}&branch=pr-1`,
        {
          body: new Uint8Array([1]),
          headers: { Authorization: `Bearer ${TOKEN}`, "Content-Type": "application/octet-stream" },
          method: "PUT",
  ...
  it("extracts branch names from Turbo team slugs", async ({ expect }) => {
    ...
    expect(put.status).toBe(200);
    expect(
      await env.ARTIFACTS.head(`v1/team/${TEAM_ID}/branch/feature-b/artifact/${artifactId}`),
    ).not.toBeNull();
  });
  ...
  it("supports the optional KV artifact store", async ({ expect }) => {
    ...
    const env = {
      ARTIFACT_STORE: "kv",
      ARTIFACTS: (workerEnv as unknown as Env).ARTIFACTS,
      ARTIFACTS_KV: kv as unknown as KVNamespace,
      TURBO_TOKEN: TOKEN,
    } satisfies Env;
    ...
    const lookup = await handleRequest(
      new Request(`${BASE_URL}${ARTIFACTS_PATH}?teamId=${TEAM_ID}`, {
        body: JSON.stringify({ hashes: [artifactId, "missing"] }),
  ...
  it("requires explicit KV binding for KV artifact store", async ({ expect }) => {
    ...
    expect(response.status).toBe(503);
    expect(await response.json()).toEqual({
      error: { code: "unavailable", message: "KV artifact store is not configured" },
    });
  });
  ...
  it("caps KV artifact uploads", async ({ expect }) => {
    const response = await handleRequest(
      ...
      }),
      {
        ARTIFACT_STORE: "kv",
        ARTIFACTS: (workerEnv as unknown as Env).ARTIFACTS,
        ARTIFACTS_KV: new MemoryKV() as unknown as KVNamespace,
        TURBO_TOKEN: TOKEN,
      },
      createExecutionContext(),
    );
    ...
    expect(await response.json()).toEqual({
      error: {
        code: "artifact_too_large",
        message: "Artifact upload exceeds configured KV size limit",
      },
    });
  });
  ...
  it("reports and purges KV team artifacts", async ({ expect }) => {
    ...
    await kv.put(`v1/team/${TEAM_ID}/artifact/one`, new Uint8Array([1, 2]), {
      metadata: kvMetadata({ duration: "1" }, 2),
    });
    await kv.put(`v1/team/${OTHER_TEAM_ID}/artifact/two`, new Uint8Array([3]), {
      metadata: kvMetadata({ duration: "1" }, 1),
    });
    ...
    expect(await purge.json()).toEqual({ deleted: 1, team: TEAM_ID, truncated: false });
  ...
  it("supports no-team single-tenant artifact requests", async ({ expect }) => {
    ...
    const stored = await (workerEnv as unknown as Env).ARTIFACTS.head(
      `v1/team/global/artifact/${artifactId}`,
    );
  ...
  it("indexes artifact metadata when an index database is bound", async ({ expect }) => {
    ...
    const response = await handleRequest(
      ...
      {
        ARTIFACT_INDEX: index as unknown as D1Database,
        ARTIFACTS: (workerEnv as unknown as Env).ARTIFACTS,
        TURBO_TOKEN: TOKEN,
      },
      ctx,
  ...
  it("does not fail uploads when artifact indexing fails", async ({ expect }) => {
    const response = await handleRequest(
      ...
      {
        ARTIFACT_INDEX: new ThrowingD1() as unknown as D1Database,
        ARTIFACTS: (workerEnv as unknown as Env).ARTIFACTS,
        TURBO_TOKEN: TOKEN,
      },
      createExecutionContext(),
  ...
  it("rejects unsupported upload content types", async ({ expect }) => {
    const response = await fetchAuthed(
      `${ARTIFACTS_PATH}/${randomArtifactId()}?teamId=${TEAM_ID}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: new Uint8Array([1]),
  ...
  it("treats duplicate puts as last-writer-wins", async ({ expect }) => {
    ...
    const second = await fetchAuthed(`${ARTIFACTS_PATH}/${artifactId}?teamId=${TEAM_ID}`, {
      ...
      headers: {
        "Content-Type": "application/octet-stream",
        "x-artifact-duration": "2",
        "x-artifact-tag": "second",
      },
  ...
  it("handles R2 objects without custom metadata", async ({ expect }) => {
    const artifactId = randomArtifactId();
    await (workerEnv as unknown as Env).ARTIFACTS.put(
      `v1/team/${TEAM_ID}/artifact/${artifactId}`,
      new Uint8Array([1, 2]),
      {
        httpMetadata: { contentType: "application/octet-stream" },
  ...
  it("serves authenticated cache api hits after R2 fill", async ({ expect }) => {
    ...
    const cachedArtifact = await artifactCache().match(cacheRequest(key));
    expect(cachedArtifact).not.toBeUndefined();
  });

  it("invalidates cache api entries when artifacts are overwritten", async ({ expect }) => {
    const artifactId = randomArtifactId();
    const key = `v1/team/${TEAM_ID}/artifact/${artifactId}`;
    const env = {
      ARTIFACTS: (workerEnv as unknown as Env).ARTIFACTS,
    ...
    await env.ARTIFACTS.put(key, new Uint8Array([1]), {
      httpMetadata: { contentType: "application/octet-stream" },
    });
    ...
    const overwrittenArtifact = await artifactCache().match(cacheRequest(key));
    expect(overwrittenArtifact).toBeUndefined();
  ...
  it("keeps cache api entries scoped by team", async ({ expect }) => {
    ...
    await (workerEnv as unknown as Env).ARTIFACTS.put(
      `v1/team/${OTHER_TEAM_ID}/artifact/${artifactId}`,
      new Uint8Array([2]),
      {
        httpMetadata: { contentType: "application/octet-stream" },
        customMetadata: { duration: "0" },
    ...
    await handleRequest(
      new Request(`${BASE_URL}${ARTIFACTS_PATH}/${artifactId}?teamId=${OTHER_TEAM_ID}`, {
        headers: { Authorization: `Bearer ${TOKEN}` },
      }),
      {
        ARTIFACTS: (workerEnv as unknown as Env).ARTIFACTS,
    ...
    await (workerEnv as unknown as Env).ARTIFACTS.delete(
      `v1/team/${TEAM_ID}/artifact/${artifactId}`,
    );
    await (workerEnv as unknown as Env).ARTIFACTS.delete(
      `v1/team/${OTHER_TEAM_ID}/artifact/${artifactId}`,
  ...
  it("skips cache api fill above size threshold", async ({ expect }) => {
    ...
    expect(first.status).toBe(200);
    const cachedArtifact = await artifactCache().match(cacheRequest(key));
    expect(cachedArtifact).toBeUndefined();
  ...
  it("scopes slug and teamId to separate R2 keys", async ({ expect }) => {
    ...
    const slugObject = await (workerEnv as unknown as Env).ARTIFACTS.head(
      `v1/team/docs/artifact/${artifactId}`,
    );
    const teamIdObject = await (workerEnv as unknown as Env).ARTIFACTS.head(
      `v1/team/team_docs/artifact/${artifactId}`,
    );
    expect(slugObject).not.toBeNull();
    expect(teamIdObject).not.toBeNull();
  });

  it("canonicalizes tenant aliases with the same value", async ({ expect }) => {
    const artifactId = randomArtifactId();
    await fetchAuthed(`${ARTIFACTS_PATH}/${artifactId}?slug=${TEAM_ID}`, {
      method: "PUT",
      headers: { "Content-Type": "application/octet-stream" },
      body: new Uint8Array([1]),
    });
    ...
    expect(get.status).toBe(200);
    expect(new Uint8Array(await get.arrayBuffer())).toEqual(new Uint8Array([1]));
  });
  ...
  it("accepts Turbo analytics events", async ({ expect }) => {
    const response = await fetchAuthed(`${ARTIFACT_EVENTS_PATH}?teamId=${TEAM_ID}`, {
      ...
      body: JSON.stringify([
        { event: "HIT", hash: randomArtifactId(), source: "REMOTE", duration: 1 },
      ]),
    });
  ...
  it("rejects malformed Turbo analytics events", async ({ expect }) => {
    ...
    expect(await response.json()).toEqual({
      error: {
        code: "bad_request",
        message: "Events request body must be an array of Turbo cache events",
      },
    });
  });
  ...
  it("emits analytics datapoints without blocking requests", async ({ expect }) => {
    ...
    const analytics = {
      writeDataPoint: (point: AnalyticsPoint) => points.push(point),
    ...
    expect(points.find((point) => point.blobs?.[0] === "put")?.blobs?.[3]).toBe(
