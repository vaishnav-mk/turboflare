...
describe("cleanupExpiredArtifacts", () => {
  it("deletes only expired v1 artifacts", async ({ expect }) => {
    ...
    const result = await cleanupExpiredArtifacts(
      { ARTIFACTS: bucket as unknown as R2Bucket, RETENTION_DAYS: "30" } satisfies Env,
      Date.now(),
    );
  ...
  it("can be disabled with zero retention or zero max delete", async ({ expect }) => {
    ...
    expect(
      await cleanupExpiredArtifacts(
        { ARTIFACTS: bucket as unknown as R2Bucket, RETENTION_DAYS: "0" } satisfies Env,
        Date.now(),
      ),
    ).toEqual({ deleted: 0, scanned: 0 });
    expect(
      await cleanupExpiredArtifacts(
        { ARTIFACTS: bucket as unknown as R2Bucket, CLEANUP_MAX_DELETE: "0" } satisfies Env,
        Date.now(),
      ),
    ).toEqual({ deleted: 0, scanned: 0 });
