...
describe("bearer auth", () => {
	...
	it("parses scoped token rules", ({ expect }) => {
		expect(
			parseScopedTokens(
				JSON.stringify([{ id: "ci", token: "scoped-token", teams: ["team_a"], scopes: [AuthScope.Read, AuthScope.Write] }])
			)
		).toEqual([{ id: "ci", scopes: [AuthScope.Read, AuthScope.Write], teams: ["team_a"], token: "scoped-token" }]);
	});

	it("ignores invalid scoped token rules", ({ expect }) => {
		expect(parseScopedTokens(JSON.stringify([{ token: "missing-teams", scopes: [AuthScope.Read] }, { teams: ["team_a"], scopes: [AuthScope.Read] }]))).toEqual([]);
	});
});
