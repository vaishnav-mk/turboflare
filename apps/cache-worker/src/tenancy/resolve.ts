...
const BRANCH_HEADER = "x-turboflare-branch";
...
export function resolveTenant(request: Request, env: Env): TenantContext {
	...
	const explicitBranch = normalizedBranch(url.searchParams.get("branch") ?? request.headers.get(BRANCH_HEADER));
	const inferredBranch = branchFromTenantKey(baseTenant.key);
	const branch = explicitBranch ?? inferredBranch;
	const tenant = inferredBranch === undefined || explicitBranch !== undefined ? baseTenant : { ...baseTenant, key: tenantKeyWithoutBranch(baseTenant.key) };
	...
	if (branch === config.defaultBranch) {
		return config.branchCachePolicy === BranchCachePolicy.Isolated ? { ...tenant, branch, readOnly: false } : { ...tenant, readOnly: false };
	}
	...
	return config.branchCachePolicy === BranchCachePolicy.MainWritePrRead ? { ...tenant, branch, fallbackBranch: "", readOnly: false } : { ...tenant, branch, readOnly: false };
}
