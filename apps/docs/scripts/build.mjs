...
import { allPages, hrefFor, pages } from "./pages.mjs";
...
for (const page of flat) {
	const markdown = stripFrontmatter(await readFile(join(root, page.source), "utf8"));
...
async function highlightCodeBlocks(html) {
	...
	for (let i = matches.length - 1; i >= 0; i--) {
		...
		result = result.slice(0, match.index) + `<div class="code-block">${labelHtml}${copyBtn}${inner}</div>` + result.slice(match.index + match.full.length);
	}
	return result;
}
